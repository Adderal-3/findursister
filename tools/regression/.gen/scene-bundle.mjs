...
var GAP_X = 0.04;
var GAP_Y = 0.06;
var PACKING_FOOTPRINT_FACTOR = 1;
var uidCounter = 1;
...
function generateScene(spec) {
  ...
  const selectedTargetIds = /* @__PURE__ */ new Set();
  const targetEntries = /* @__PURE__ */ new Map();
  const targetTasks = [];
  for (const rule of spec.rules) {
    const targetPool = ITEMS.filter(
      (item) => !item.distractorOnly && !selectedTargetIds.has(item.id) && itemMatchesTask(item, rule)
    );
    const targetCount = Math.min(
      Math.max(1, Math.trunc(rule.targetCount)),
      targetPool.length
    );
    const preferredPool = spec.preferredTargetIds ? targetPool.filter((item) => spec.preferredTargetIds?.has(item.id)) : [];
    ...
    const remaining = shuffle(targetPool.filter((item) => !preferredIds.has(item.id))).slice(0, targetCount - preferred.length);
  ...
  const distractorPool = ITEMS.filter(
    (item) => (taskUsesLandmarks || item.role !== "landmark") && !selectedTargetIds.has(item.id) && (item.distractorOnly || !spec.rules.some((rule) => itemMatchesTask(item, rule)))
  );
  ...
  const prepared = spawnList.map((entry) => {
    ...
    const scale = (isLandmark ? rand(1, 1.2) : entry.isTarget && spec.levelType === "boss" ? rand(0.96, 1.14) : entry.isTarget ? rand(1.05, 1.34) : rand(0.9, 1.3)) * (isLandmark ? 1 : densityScale * visibilityBoost);
  ...
  for (const entry of prepared) {
    ...
    const minimumScale = entry.landmarkIndex != null ? 0.88 : entry.isTarget ? entry.def.visualTags?.includes("slender") ? 0.76 : 0.68 : entry.def.visualTags?.includes("slender") ? 0.48 : 0.4;
    let position = null;
    ...
    for (let shrinkRound = 0; shrinkRound < 14 && !position; shrinkRound += 1) {
      ...
      for (let attempt = 0; attempt < 900; attempt += 1) {
        ...
        if (entry.landmarkIndex != null) {
          ...
          const cluster = spec.levelType === "cluster" && entry.isTarget ? clusters[0] : clusters[Math.floor(rand(0, clusters.length))];
