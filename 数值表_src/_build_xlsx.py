# -*- coding: utf-8 -*-
"""构建《忙忙碌碌寻宝藏》数值表.xlsx v3.4：封面 + 5 张数值表 + 数值决议（D1~D29）"""
import csv, os
...
ws.merge_cells('B2:H2'); put(ws, 2, 2, '《忙忙碌碌寻宝藏》数值表 v3.4', F_TITLE, LEFT); ws.row_dimensions[2].height = 34
ws.merge_cells('B3:H3'); put(ws, 3, 2, '配套《玩法设计文档 v3.4》· 伙伴原名与透明计分口径 · 供关卡 / 系统 / 数值开发使用', F_SUB, LEFT)
ws.merge_cells('B5:H5'); put(ws, 5, 2, '核心规模', Font(size=13, bold=True), LEFT)
metrics = [('主线关卡', '100 关（5 篇章 / 10 小章）'), ('伙伴', '8 位 × 8 种招募方式'), ('物品素材', '123 件（隐藏首次发现，不公开数量）'),
           ('道具', '4 种（无尽模式禁用，尚未实现）'), ('无尽波次表', '40 波（含 3 个高压波）'), ('排行榜满加成', '+10%（8 位伙伴 × 1.25%）'),
...
partners = [
    (1, '叶问舟', 'kitchen', '在线时长', '累计在线 ≥ 1800 秒（30 分钟）', 'znm.prog.onlineSec', '累计', '在线 24/30 分钟'),
    (2, '燕无归', 'toys', '活跃', '单日累计游玩 ≥ 1200 秒（20 分钟）任意一天（D15：不绑连续签到）', 'znm.prog.dailyPlaySec', '单日', '单日游玩 14/20 分钟'),
    (3, '叶雪青', 'nature', '寻物', '自然物品累计发现 ≥ 15 种（不公开总发现数）', 'znm.collection.items.v1（派生）', '累计', '自然发现 11/15'),
    (4, '方承意', 'travel', '任务', '历史最高星级为三星的关卡累计 ≥ 10 关（只增不减）', 'znm.best.levelStars', '累计', '三星通关 7/10'),
    (5, '阿初', 'ocean', '积累', '累计命中目标 ≥ 500 次（含重复物品）', 'znm.prog.totalFound', '累计', '累计找物 163/500'),
    (6, '花将离', 'space', '分数', '单关基础分 ≥ 1500（不含伙伴系数）', 'znm.prog.bestBaseScore', '单次', '单关最高 1286/1500'),
    (7, '无情', '彩蛋', '技巧', '有效单局最大连击 ≥ 8', 'znm.prog.bestCombo', '单次', '最大连击 6/8'),
    (8, '姬蜜儿', '终极彩蛋', '羁绊', '其余 7 位全部招募后自动加入（不占常规判定）', 'znm.partner.recruited', '事件', '伙伴 5/7'),
]
...
heads = ['编号', '决议', '背景与影响']
wids = [7, 52, 60]
row0 = sheet_head(ws, '数值口径决议 D1~D29（开发必读）', 'D1~D8 为基础口径；D9~D28 为历史收口；D29 为 v3.4 信息层级与写榜决议', heads, wids)
decisions = [
    ...
    ('D8', '伙伴判定检查点 = 结算 / 隐藏首次发现 / onlineTick(30s) / 启动兜底', '纯挂机玩家也能按在线时长招到叶问舟'),
    ...
    ('D12', '不提供玩家可见图鉴，不产生图鉴加成；仅保留隐藏首次发现记录', 'v3.4 产品决策覆盖旧图鉴方案'),
    ('D13', '日榜 = 当日各关最高基础分之和，每关每日只计一次，不吃任何加成', '钉死口径，防止系数污染日榜'),
    ('D14', '终身/赛季总榜同分按对应基础分达到时间，系数变化不刷新时间戳', '防止后招募伙伴者靠系数偷排名'),
    ('D15', '无掉队设计：永久进度不绑自然日累计；每日上限仅限消耗品', '燕无归使用“单日游玩20分钟”达成型条件'),
    ('D16', '站内视频三入口：体力 +5 / 局中加时固定 +15s / 卡关放大镜 +1', '导流主通道；加时与沙漏共享每关≤3 次上限'),
    ('D17', '伙伴阈值：叶问舟 1800s / 燕无归单日 1200s / 叶雪青 15 件 / 方承意 10 三星 / 阿初 500 件 / 花将离 1500 分 / 无情 8 连击 / 姬蜜儿 7 人齐', '展示名严格来自 huoban 源素材文件名'),
    ('D18', 'Boss 关类型系数 0.9（L10=213s、L20 起=300s）', '替代旧 1.3，告别 7.2 分钟疲劳局'),
    ('D19', 'UGC 隔离：1 眼力/次，成绩不计榜单/招募/隐藏发现', '时长30~300s、物品≤130、目标≤已摆放实例'),
    ...
    ('D27', '历史稀有度方案取消；当前只保留未发现目标追赶式补位', '不对玩家展示发现总数或稀有度'),
    ('D28', '静态 H5 首发边界：本地玩法 + 大神平台服务', '可信榜单仍需真实后台 ID、验签与生产验收'),
    ('D29', '最终榜分 = round(各关历史最高基础分合计 × (1 + 伙伴加成))', '写 levels_total_score；另写 levels_base_score 与 partner_bonus_bps 供审计'),
]
