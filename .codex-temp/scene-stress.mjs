...
// src/game/scene.ts
var SCENE_SCALE = { w: 2.4, h: 1 };
var SCENE_ITEM_FRACTION = 0.155;
...
function crossesViewportSeam(x, scale) {
  const safeHalfWidth = BASE_X_PERCENT * scale / 2 + 4.2;
...
function generateScene(spec) {
  ...
  const prepared = spawnList.map((entry) => {
    ...
    const scale = isLandmark ? rand(1.2, 1.5) : rand(0.78, 1.22);
...
// .codex-temp/scene-stress.ts
var csv = fs.readFileSync(new URL("../\u6570\u503C\u8868_src/levels_100.csv", import.meta.url), "utf8").trim();
var [headerLine, ...lines] = csv.split(/\r?\n/);
var headers = headerLine.split(",");
var generated = 0;
var maxSkippedDistractors = 0;
